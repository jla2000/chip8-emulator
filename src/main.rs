...
struct Cpu {
    memory: [u8; 4096],
    stack: Vec<u16>,
    graphics: [u8; CELL_ROWS * CELL_COLUMNS],
    regs: [u8; 16],
    keys: [bool; 16],
    pc: usize,
    i: u16,
...
fn nibbles(value: u16) -> (u8, u8, u8, u8) {
    (
...
impl Cpu {
    fn new() -> Self {
        ...
        Self {
            memory,
            stack: vec![],
            graphics: [0x00; CELL_ROWS * CELL_COLUMNS],
            regs: [0x00; 16],
            keys: [false; 16],
            pc: 0x0000,
            i: 0x0000,
    ...
    fn load_rom(&mut self, rom_bytes: &[u8]) {
    ...
    fn emulate_cycle(&mut self) {
        ...
        match nibbles(opcode) {
            ...
            (0xD, vx, vy, n) => {
                let x = self.regs[vx as usize] as usize % CELL_ROWS;
                let y = self.regs[vy as usize] as usize % CELL_COLUMNS;
                ...
                for y_offset in 0..height {
                    ...
                    for x_offset in 0..8usize {
                        if sprite_byte & (0x80 >> x_offset) != 0 {
                            let dst_pixel = (y + y_offset) * CELL_ROWS + x + x_offset;
...
fn main() {
    let rom = include_bytes!("picture.ch8");
    let mut cpu = Cpu::new();
    cpu.load_rom(rom);
    cpu.run();
}
